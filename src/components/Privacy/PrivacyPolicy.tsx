プライバシーポリシー個人情報の取り扱いについてキャリアスコア診断（以下「当サービス」）では、ご利用者様の個人情報を以下の目的でのみ使用いたします。収集する情報診断結果（スコア、ランク、回答内容）お名前メールアドレス相談予約時にご記入いただいた情報利用目的診断結果の提供無料相談の実施提携する人材紹介会社への情報共有（より適切な転職支援を提供するため）サービス改善のための統計分析第三者への提供ご相談いただいた内容は、あなたの転職支援を目的として、提携する人材紹介会社と共有させていただきます。共有する情報診断結果相談時にお聞きした転職希望条件ご経歴の概要情報の管理お預かりした個人情報は、適切に管理し、上記目的以外には使用いたしません。の使用当サービスでは、サービスの利便性向上のためを使用する場合があります。の使用を希望されない場合は、ブラウザの設定で無効にすることができます。お問い合わせ個人情報の取り扱いに関するご質問は、以下までご連絡ください。メール最終更新日年月日トップページに戻るexport default function PrivacyPolicy() {
  return (
